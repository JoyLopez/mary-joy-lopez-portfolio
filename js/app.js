...
(function(){
  /* ------------------------------------------------------------
     Joy AI runs fully client-side on a local knowledge base
  ...
  const CONTACT = {
    ...
    facebook: 'https://www.facebook.com/maryjoy.lopez.50364592',
    resume: 'documents/Mary_Joy_Lopez_Software_QA_Engineer_CV.pdf',
  ...
  function saveHistory(){
    try{ localStorage.setItem('joyAiHistory', JSON.stringify(history)); }catch(e){}
  }
  function loadHistory(){
    try{ return JSON.parse(localStorage.getItem('joyAiHistory') || '[]'); }catch(e){ return []; }
  }
  ...
  function recruiterNode(){
    return actionRow([
      { icon:'fa-solid fa-download', label:'Download Resume', onClick:() => window.open(CONTACT.resume,'_blank') },
      { icon:'fa-solid fa-envelope', label:'Contact Joy', onClick:() => window.open('mailto:'+CONTACT.email) },
    ]);
  }
  ...
  const SECTIONS = [
    ...
    { id:'about',        selector:'#about',        label:'About',        keywords:['about','about me','about joy','who is joy','bio'] },
  ...
  function getResponse(rawQuery){
    ...
    // --- Special commands ---
    if(has('resume', 'cv')) return { text: `Here's Joy's resume — up to date with her QA and automation background.`, node: resumeNode(), icon:'fa-solid fa-download' };
    if(has('github')) return { text: `Here's Joy's GitHub profile.`, node: githubNode(), icon:'fa-brands fa-github' };
    if(has('linkedin')) return { text: `Here's Joy's LinkedIn profile.`, node: linkedinNode(), icon:'fa-brands fa-linkedin' };
    if(has('email', 'e-mail')) return { text: `You can reach Joy directly at **${CONTACT.email}**.`, node: emailNode(), icon:'fa-solid fa-envelope' };
    if(has('contact', 'reach her', 'get in touch', 'hire')) return {
      text: `Best ways to reach Joy:\n- Email: **${CONTACT.email}**\n- LinkedIn or GitHub (buttons below)\n- Or use the contact form on this page`,
      node: actionRow([
    ...
    if(has('project')) return { text: `Here are a few things Joy has built and tested:`, node: projectCardsNode(), icon:'fa-solid fa-diagram-project' };

    // --- Recruiter quick summary ---
    if(has('recruiter', 'summary', 'overview', 'quick look', 'tell me about joy', 'who is joy', 'about joy')) return {
      text: `**Mary Joy Lopez** — Software QA Engineer & Computer Engineer\n\n` +
      ...
      icon:'fa-solid fa-id-badge'
        ...
        if(has('tae', 'summary', 'overview', 'quick look', 'tell me about carlo', 'who is joy', 'about joy')) return {
      text: `**Mary Joy Lopez** — Software QA Engineer & Computer Engineer\n\n` +
        `- **Experience:** ~3 years across manual and automation QA\n` +
        `- **Core strengths:** ${topSkills(4).join(', ')}\n` +
        `- **QA expertise:** functional, regression, exploratory, UI, and cross-browser testing\n` +
    ...
    if(has('qa experience', 'quality assurance', 'manual test', 'test experience', 'testing experience')) return {
      text: `Joy has around 3 years of hands-on QA experience:\n` +
    ...
    if(has('automation testing', 'automated test', 'playwright')) return {
      text: `Joy builds test automation with **Playwright** and **Python**, structured as modular, CI-integrated frameworks for regression and API testing — the kind of setup that scales past a handful of scripts.`,
      icon:'fa-solid fa-robot'
    };
    if(has('api test')) return { text: `Yes — Joy does API testing as part of her regression and QA automation work, verifying endpoints alongside UI flows.`, icon:'fa-solid fa-plug' };
    if(has('regression')) return { text: `Regression testing is one of Joy's core skills — she maintains detailed regression suites (${kbSkills.find(s=>s.name==='Regression Testing')?.level ?? 90}% proficiency) to make sure new changes don't break existing functionality.`, icon:'fa-solid fa-rotate' };
    if(has('exploratory')) return { text: `Exploratory testing is where Joy digs in without a script — probing edge cases and unexpected flows that formal test cases might miss.`, icon:'fa-solid fa-compass' };

    // --- Skills / languages ---
    if(has('programming language', 'languages does he know', 'what language')) return {
      text: `Joy works with **JavaScript**, **Python**, **HTML**, **CSS**, and **SQL** — plus tools like **Git**, **GitHub**, **Jira**, and **Playwright** for testing and automation.`,
      icon:'fa-solid fa-code'
    };
    if(has('skill')) return {
      text: `Joy's core skill set:\n` +
        kbSkills.map(s => `- **${s.name}** — ${s.level}%`).join('\n'),
      icon:'fa-solid fa-layer-group'
    };
    if(has('git', 'version control')) return { text: `Joy uses **Git** and **GitHub** daily for version control and collaboration on both dev and QA projects.`, icon:'fa-brands fa-git-alt' };
    if(has('jira')) return { text: `Joy uses **Jira** for defect tracking, sprint planning, and coordinating with dev teams during releases.`, icon:'fa-brands fa-jira' };
    if(has('html') || has('css')) return { text: `Joy is comfortable with **HTML** and **CSS** for building and testing responsive front-ends — this very site is a good example.`, icon:'fa-brands fa-html5' };
    if(has('sql') || has('database')) return { text: `Joy has working SQL knowledge for querying and validating data during backend and integration testing.`, icon:'fa-solid fa-database' };
    ...
    if(has('current job', 'currently working', 'current role', 'current company')) {
      ...
      return { text: `Joy is currently a **${cur.role}** at **${cur.company}** (${cur.date}).`, icon:'fa-solid fa-briefcase' };
    }

    // --- Education / certifications (not provided — be honest, no fabrication) ---
    if(has('education', 'degree', 'university', 'college')) return {
      text: `Joy studied Computer Engineering. For specific degree details, it's best to ask her directly — feel free to use the contact options below.`,
      node: emailNode(),
      icon:'fa-solid fa-graduation-cap'
    };
    if(has('certification', 'certificate', 'certified')) return {
      text: `Joy hasn't listed formal certifications on this site yet. If that's important for your role, reach out directly and she can share the latest.`,
      node: emailNode(),
    ...
    if(has('hello','hi ','hey','good morning','good afternoon','yo') || q.trim()==='hi' || q.trim()==='hey') return {
      text: `Hey! 👋 I'm **Joy AI** — ask me about her QA experience, skills, projects, or how to get in touch.`,
      icon:'fa-solid fa-hand-sparkles'
    };
    if(has('thank')) return { text: `You're welcome! Anything else you'd like to know about Joy?`, icon:'fa-solid fa-face-smile' };
    if(has('bug')) return { text: `Fitting question for a QA portfolio 😄 — Joy's whole job is finding these before your users do. Want to see her testing experience or automation projects?`, icon:'fa-solid fa-bug' };

    // --- Fallback: question isn't related to this page/portfolio ---
    return {
      text: `That doesn't seem related to Joy's portfolio, so I can't help with it here.\n\n` +
        `I can:\n` +
        `- Answer questions about Joy's **skills**, **QA/testing experience**, **projects**, or **work history**\n` +
        `- **Navigate this page for you** — just say something like *"take me to your projects"* or *"go to the contact section"*\n` +
        `- Help you **contact** Joy or grab her **resume**\n\n` +
  ...
  function openChat(){
    ...
    localStorage.setItem('joyAiSeen','1');
  ...
  if(localStorage.getItem('joyAiSeen') === '1'){ badge.style.display = 'none'; }
  ...
  const setProfilePhoto = (isLight) => {
    if(profilePhoto) profilePhoto.src = isLight ? 'assets/images/profile/profile-image.jpg' : 'assets/images/profile/profile-image.jpeg';
  };
  themeBtn.addEventListener('click', () => {
    ...
    localStorage.setItem('joyAiTheme', isLight ? 'dark' : 'light');
    setProfilePhoto(!isLight);
  });
  const savedTheme = localStorage.getItem('joyAiTheme');
  if(savedTheme === 'light'){
  ...
  function seedWelcome(){
    ...
    const html = renderMarkdown(
      `Hi, I'm **Joy AI** 👋\n\nI can walk you through Joy's QA background, automation skills, projects, and how to reach her — just ask, or tap a suggestion below.`
    );
